...
components/
  ...
  04_network/server/   — ServerModule + StaticHandler (SPIFFS) + WsHandler (WS)
  ...
  05_storage/spiffs_service/ — обёртка esp_spiffs (mount/read/write/exists)
...
|---|---|---|---|
| `config` (ConfigStore) | 10 | true | конфиг нужен всем, монтирует SPIFFS |
...
Кастомная таблица разделов `partitions_new.csv` (factory app 1MB + SPIFFS
`storage` 1MB) фиксируется в `sdkconfig.defaults`.
...
- Дефолты можно менять через `/spiffs/config.json`
...
`spiffs_image/` содержит СБОРОЧНЫЙ bundle Vue3 из отдельного репозитория
(`pavellzubkov/vue3_embedded`). Текущий фронт **не понимает** новый бинарный
протокол — новый фронт пересобирается отдельно под `docs/PROTOCOL-J1939.md`.
